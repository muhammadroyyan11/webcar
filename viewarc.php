...
<html>
<head>
<title>Archived CAR Files</title>
...
include ("redirectlogin.php");
	...
	$displayItem = "";
		...
		if ($selectResult)
		{
			...
			$displayItem .= "<th>CAR Number</th><th>CAR Status</th><th>Departement</th><th></th></tr>";
			}
...
<div align="center">
  ...
  <?php
   $form = "<center>";
	$form .= "<form method=\"POST\" action=\"viewarc.php\">";
	$form .= "<table class =\"style1\">";
	$form .= "<tr>";
	
	$form .= "<td>Number</td><td><select name=\"sinum\"><option value=\"\"></option>";
$sin = "select INumber from file";
...
$form .= "</select></td>";
    ...
    $g = "select distinct DeptID from file";
	$gr = mysql_query($g);
	if($gr)
	{
	while($sr = mysql_fetch_array($gr))
	{
		$g1 = $sr['DeptID'];
		$getDept = "select DeptID,DeptName from tblDept where DeptID= $g1 ORDER BY DeptName ASC";
    $getDeptResult = mysql_query($getDept);
    ...
    $form .= "</select></td>";
	$form .= "<td><input type=\"hidden\" name=\"what\" value=\"search\"><input type=\"hidden\" name=\"group\" value=\"$group\">";
	$form .= "<input type=\"hidden\" name=\"id\" value=\"$id11\"></td><td><input type=\"submit\" value=\"Search\"></td>";
	$form .= "</tr>";
	$form .= "</table>";
	$form .= "</form>";
	print $form;
   ...
   if($displayItem !="")
   {
    print "<div style=\"width: 900px; height: 500px;overflow: auto; padding: 5px\">".$displayItem; 
 }
   ...
   <table width="78" border="0">
     <tr>
	   ...
	   <form name="form2" method="post" action="viewarc.php">
	   <input type="hidden" name="group" value="<?php echo $group; ?>">
	   <input type="hidden" name="id" value="<?php echo $id11; ?>">
...
{ 
     ...
     if ($offset < $fileCount) { 
          ...
          $displayNext .= "<td><form name=\"form1\" method=\"post\" action=\"viewarc.php\">";
		  $displayNext .= "<input type=\"hidden\" name=\"group\" value=\"$group\">";
		  $displayNext .= "<input type=\"hidden\" name=\"id\" value=\"$id11\">";
...
{ 
     ...
     { 
          ...
          $displayPrevious .= "<td><form name=\"form1\" method=\"post\" action=\"viewarc.php\">";
