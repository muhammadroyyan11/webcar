...
$count = 0;
			$se = "select Name from tblMUAccount where MUserID = $id";
			$ser = mysql_query($se);
			while ($rn = mysql_fetch_array($ser))
			{
			$n = $rn['Name'];
			}
			
			$select1 = "select * from tblRequest where RTo = '$n' or RFrom = '$n' ORDER BY RequestID DESC LIMIT $offset,30";
	...
	if($select1Result)
	{
	    ...
	    if($totalRows != 0)
		{
			$display = "<table align = \"center\" class =\"tbl\"><tr><th class=\"t2\">Request Date</th><th class=\"t2\">To</th><th class=\"t2\">Originator Manager</th>";
			$display .= "<th class=\"t2\">Auditee Manager</th><th class=\"t2\">From</th><th class=\"t2\">Findings</th><th></th></tr>";
		}
		...
		while($row = mysql_fetch_array($select1Result))
		{
			...
			if($gmbccr)
			{
		        ...
		        $display .= "<form method=\"post\" action=\"createcar2.php\">";
				...
				$display .= "<input type=\"hidden\" name=\"what\" value=\"addinfo\">";
				$gun = "select Name from tblMUAccount where MUserID = $id";
				$gunr = mysql_query($gun);
				if($gunr)
				{
				while ($runm = mysql_fetch_array($gunr))
				{
				$unm =  $runm['Name'];
				}
				}
				if($rfrom == $unm)
				{
				$display .= "<td class=\"t1\"><input type =\"submit\" value=\"Create CAR\"></td>";
				}
				$display .= "</tr>";
				$display .= "</form>";
				$offset++;
...
{ 
	 ...
	 { 
          ...
          $displayNext .= "<td><form name=\"form1\" method=\"post\" action=\"osrequest.php\">";
...
{ 
     ...
     { 
          ...
          $displayPrevious .= "<td><form name=\"form1\" method=\"post\" action=\"osrequest.php\">";
...
{ 
   $id = $_POST['id'];
   $countQuery = "SELECT count(RequestID) as requestCount FROM tblRequest where RTo = $id or RCC = $id or RBCC = $id" ; 
