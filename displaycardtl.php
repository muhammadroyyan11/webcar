...
include ("conn.php");
		...
		if($_POST['what'] == "delete")
   		{
            $delete = "DELETE FROM tblCAR WHERE IssNumber = '$inum'";
		    $delqry = mysql_query($delete);
		    if ($delqry) 
			{
				$delete2 = "DELETE FROM tblCARDTL1 WHERE IssNumber = '$inum'";
				$delqry2 = mysql_query($delete2);
				if($delqry2)
				{
					$delete3 = "DELETE FROM tblCARDTL2 WHERE IssNumber = '$inum'";
					$delqry3 = mysql_query($delete3);
					if($delqry3)
					{
					    $delete4 = "DELETE FROM tblPIC where IssNumber = '$inum'";
						$delqry4 = mysql_query($delete4);
						if($delqry4)
						{
						//delete attachment
						$filename = "";
						$gfn = "select FileName from tblAttach where IssNumber ='$inum'";
						$gfnr = mysql_query($gfn);
						if($gfnr)
						{
						while ($rfn = mysql_fetch_array($gfnr))
						{
						$filename = $rfn['FileName'];
						
						$filedir = "192.168.1.36\attachment\\$filename"; //path to directory of actual files
				        $deletefiles = unlink ($filedir); //delete files 
						if($deletefiles)
						{
						$delete5 = "DELETE FROM tblAttach where FileName ='$filename'";
						...
						print "<p align=\"center\" class = \"style2\">Selected CAR has been deleted</p>";
			?>
...
<table border="0" align="left">
    <tr>
      <td width="74"><form name="form1" method="post" action="displaycardtl.php">
	    <input type="hidden" name="group" value="<?php echo $group; ?>">
...
<table border="0" align="left">
    <tr>
      <td width="74"><form name="form1" method="post" action="carlist.php">
	    <input type="hidden" name="group" value="<?php echo $group; ?>">
...
<?php
			...
			{
?>
<td valign="top"><table border="1"><tr><td class="style11">Attachment</td></tr><tr><td class="style11">
<?php
//display files
	$afid = "";
	$fn = "";
...
if($select5Result)
		{
			$display = "<table border=\"1\" class=\"style1\"><tr><th>Name</th><th>Departement</th></tr>";
		...
		print "<p class=\"style2\">Corrective Action Team</p>";
			...
			*/
			while($row = mysql_fetch_array($select6Result))
			{
				...
				*/
				...
				$display2 .= "<td class = \"style11\">Verification by Originator</td>";
