...
<html>
<head>
<title>Originator Manager</title>
...
include ("redirectlogin.php");
    ...
    $offsetOriginal = $offset; 
		...
		if(isset($_POST['what']))
		{
			...
			{
				$select .=" WHERE MUsername='$uname' AND Name LIKE '%$nm%' AND DeptID = $dpt AND GroupID = 'om'";
			}
		
			//username is not empty
			if(($_POST['uname'] != "") && ($_POST['nm'] == "")&& ($_POST['sdept'] == ""))
			{
				$select .=" WHERE MUsername='$uname' AND GroupID = 'om'";
			}
		
			//name is not empty
			if(($_POST['uname'] == "") && ($_POST['nm'] != "")&& ($_POST['sdept'] == ""))
			{
				$select .=" WHERE Name LIKE '%$nm%' AND GroupID = 'om'";
			}
		
			//departemen is not empty
			if(($_POST['uname'] == "") && ($_POST['nm'] == "")&& ($_POST['sdept'] != ""))
			{
				$select .=" WHERE DeptID = $dpt AND GroupID = 'om'";
			}
		
			//username and name are not empty
			if(($_POST['uname'] != "") && ($_POST['nm'] != "")&& ($_POST['sdept'] == ""))
			{
				$select .=" WHERE MUsername='$uname' AND Name LIKE '%$nm%' AND GroupID = 'om'";
			}
			
			//username and departement are not empty
			if(($_POST['uname'] != "") && ($_POST['nm'] == "")&& ($_POST['sdept'] != ""))
			{
				$select .=" WHERE MUsername='$uname' AND DeptID = $dpt AND GroupID = 'om'";
			}
		
			//name and departemen are not empty
			if(($_POST['uname'] == "") && ($_POST['nm'] != "")&& ($_POST['sdept'] != ""))
			{
				$select .=" WHERE Name LIKE '%$nm%' AND DeptID = $dpt AND GroupID = 'om'";
			}
			
			if(($_POST['uname'] == "") && ($_POST['nm'] == "")&& ($_POST['sdept'] == ""))
			{
				$select .=" WHERE GroupID = 'om'";
			}
    ...
    else
	{
		...
		$select = "SELECT * FROM tblMUAccount WHERE GroupID = 'om' ORDER BY Name ASC LIMIT $offset,50";
	}
...
<p></p>
  ...
  <?php
  $form = "<center>";
	$form .= "<form method=\"POST\" action=\"omngr.php\">";
	$form .= "<table class =\"style1\">";
	$form .= "<tr>";
	$form .= "<td>Username :</td><td><select name=\"uname\"><option value =\"\"></option>";
	$gu = "select MUsername from tblMUAccount where GroupID ='om'";
	...
	$gn = "select Name from tblMUAccount where GroupID ='om'";
    ...
    $getDept = "select DeptID from tblMUAccount where GroupID = 'om'";
    $getDeptResult = mysql_query($getDept);
   ...
   <table width="78" border="0">
     <tr>
       <td><form name="form2" method="post" action="omngr.php">
	   <input type="hidden" name="group" value="<?php echo $group; ?>">
...
{ 
     ...
     if ($offset < $staffCount) { 
          ...
          $displayNext .= "<td><form name=\"form1\" method=\"post\" action=\"omngr.php\">";
...
{ 
     ...
     { 
          ...
          $displayPrevious .= "<td><form name=\"form1\" method=\"post\" action=\"omngr.php\">";
...
{ 
    ...
    $countQuery = "SELECT count(MUserID) as staffCount FROM tblMUAccount WHERE GroupID = 'om'";
