...
if(isset($_POST['what']))
{
	if($_POST['what'] == "update")
	{
		...
		$update = "update tblPIC set PICStat = '$action' where IssNumber='$inum' AND PICID=$catid";
		$updateResult = mysql_query($update);
		if($updateResult)
		{
			print "<p class=\"style2\">Selected Corrective Action Team status has been updated</p>"; 
		}
		else
		{
			print mysql_error();
...
<table border="0" class="style1">
	...
	<td><?php print $form3;?></td>
      <td><form name="form1" method="post" action="cat.php">
			...
			while($row = mysql_fetch_array($select7Result))
			{
				...
				if($select8Result)
				{
						...
						while($row1 = mysql_fetch_array($select8Result))
						{
							...
							//$display3 .= "<form method=\"post\" action=\"catcardetail.php\">";
							$display3 .= "<tr align = \"center\">";
				            $display3 .= "<td class = \"t1\">$picName</td><td class = \"t1\">$deptName5</td><td class = \"t1\">$catSec</td>";//<td>$picStat</td>";
							
							if($pic == $id11)
							{
							$display3 .= "<td><select name=\"action\"><option value=\"approved\" selected>approve</option><option value=\"rejected\">reject</option></select></td>";
				            $display3 .= "<input type=\"hidden\" name=\"group\" value=\"$group\">";
							$display3 .= "<input type=\"hidden\" name=\"id\" value=\"$id11\">";
		                    $display3 .= "<input type=\"hidden\" name=\"inum\" value=\"$issNumber1\">";
				            $display3 .= "<input type=\"hidden\" name=\"catid\" value=\"$pic\">";
				            $display3 .= "<input type=\"hidden\" name=\"what\" value=\"update\">";
		                    $display3 .= "<td><input type =\"submit\" value=\"Submit\"></td>";
							}
			...
			while($row = mysql_fetch_array($select6Result))
			{
				...
				*/
				...
				while($rca = mysql_fetch_array($gcar))
				{
					...
					if($caid != "")
					{
					$display2 .= "<form method = \"post\" action = \"editca.php\"><tr>";
					$display2 .= "<td valign=\"top\">$ac</td><td valign=\"top\">$dbnm</td><td valign=\"top\">$cas</td><td valign=\"top\">";
					if($cas == "closed")
					{
					$display2 .= "$cafd";
					}
					$display2 .= "</td><input type =\"hidden\" name=\"group\" value=\"$group\">";
				    $display2 .= "<input type =\"hidden\" name=\"id\" value=\"$id11\"><input type =\"hidden\" name=\"inum\" value=\"$inum\">";
					...
					if($cas != "closed")
					{
					$display2 .= "<td><input type=\"submit\" value=\"Edit/Delete\"></td>";
					}
					$display2 .= "</tr></form>";
					}
				...
				*/
				...
				$display2 .= "<form method=\"post\" action=\"catedit.php\">";
			    ...
			    $display2 .= "<tr><td colspan=\"7\">";
				if($status != "closed")
				{
				$display2 .= "<input type =\"submit\" value=\"Add/Edit Analysis\">";
				}
				$display2 .= "</form>";
				if($analysis != "")
				{
				$display2 .= "<form method=\"post\" action=\"catedit1.php\"><input type =\"hidden\" name=\"group\" value=\"$group\">";
				$display2 .= "<input type =\"hidden\" name=\"id\" value=\"$id11\"><input type =\"hidden\" name=\"inum\" value=\"$inum\">";
				$display2 .= "<input type =\"hidden\" name=\"cardtlid2\" value=\"$cardtlid2\"><input type =\"hidden\" name=\"what\">";
				if($status != "closed")
				{
				$display2 .= "<input type =\"submit\" value=\"Add Corrective Action\">";
				}
				$display2 .= "</form>";
				}
				$display2 .= "</td></tr>";
