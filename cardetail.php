...
if(isset($_POST['what']))
{
	...
	$editstatus2 = $_POST['editstatus'];
$ucs2 = "UPDATE tblCAR SET CARStatus='$editstatus2' where IssNumber = '$inum'";
$updateResult3 = mysql_query($ucs2);

	if($_POST['what'] == "editstat")
	{
	$caid1 = $_POST['caid'];
	$castat = $_POST['castatus'];
	
	$ucs = "update tblCA set CAStatus = '$castat',CADate = $cdate ,CAMonth = $cmonth ,CAYear = $cyear where CAID = $caid1";
	$ucsr = mysql_query($ucs);
	if($ucsr)
	{
		$check = "select CAStatus from tblCA where IssNumber = '$inum'";
		$checkResult = mysql_query($check);
		if($checkResult)
		{
		while($rstat = mysql_fetch_array($checkResult))
		{
			$cast = $rstat['CAStatus'];
			if($cast == "open")
			{
		...
		if($close == 0)
		{
		//update status header to closed
		$updateStatus1 = "UPDATE tblCAR SET CARStatus='closed',CDate =$cdate,CMonth=$cmonth,CYear=$cyear WHERE IssNumber='$inum'";
		$updateResult1 = mysql_query($updateStatus1);
		}
		else
		{
		$updateStatus2 = "UPDATE tblCAR SET CARStatus='open' WHERE IssNumber='$inum'";
		$updateResult2 = mysql_query($updateStatus2);
		}
		}
		
		print "<p class=\"style2\">Status has been updated</p>";
	}
	}
	
	/*
	if($_POST['what'] == "carstatus")
	{
		$carstatus = $_POST['status'];
		
		$updateStatus = "UPDATE tblCAR SET CARStatus='$carstatus',CDate =$cdate,CMonth=$cmonth,CYear=$cyear WHERE IssNumber='$inum'";
		$updateResult = mysql_query($updateStatus);
		if($updateResult)
		{
		print "<p class=\"style2\">CAR Status has been updated</p>";
		}
	} //close if($_POST['what'] == "carstatus")
	*/
}
...
<table border="0" align="left">
    <tr>
      <td width="74"><form name="form1" method="post" action="os.php">
	...
	?>
	<!--
      <td width="74"><form name="form1" method="post" action="editom.php">
	    <input type="hidden" name="group" value="<?php //echo $group; ?>">
		 <input type="hidden" name="id" value="<?php //echo $id11; ?>">
		<input type="hidden" name="inum" value="<?php //echo $issNumber1; ?>">
		<input type="hidden" name="view" value="<?php //echo $view; ?>">
        <input type="submit" name="Submit2" value="Edit Originator Manager">
      </form></td>
	   <td width="74"><form name="form1" method="post" action="editam.php">
	   <input type="hidden" name="group" value="<?php //echo $group; ?>">
		 <input type="hidden" name="id" value="<?php //echo $id11; ?>">
		<input type="hidden" name="inum" value="<?php //echo $issNumber1; ?>">
		<input type="hidden" name="view" value="<?php //echo $view; ?>">
        <input type="submit" name="Submit2" value="Edit In Charge Manager">
      </form></td>
	   <td width="74"><form name="form1" method="post" action="editcam.php">
	    <input type="hidden" name="group" value="<?php //echo $group; ?>">
		 <input type="hidden" name="id" value="<?php //echo $id11; ?>">
		<input type="hidden" name="inum" value="<?php //echo $issNumber1; ?>">
		<input type="hidden" name="view" value="<?php //echo $view; ?>">
        <input type="submit" name="Submit2" value="Edit Corrective Action Manager">
      </form></td>
	  -->
	  <?php
	  }
...
<input type="hidden" name="what" value="carstatus">
...
<table border="0" class="tbl">
	...
	?>
		  ...
		  <td class = "t2"><strong>Edit Status :</strong></td>
			<?php  if($status == "open")
		  {
            ...
            <td class = "t1">
			<form method="post" action="cardetail.php"  >
			<select name="editstatus"><option value="open" selected>open</option><option value="cancel">cancel</option>
		...
		if ($status == "cancel") {
            ...
            <td class = "t1">
			<form method="post" action="cardetail.php"  >
			<select name="editstatus"><option value="cancel" selected>cancel</option><option value="open">open</option>
			  </select><input type="submit" value="update"></td>
          </tr></form>
		  <?php
		  }
		 ...
		 if($status == "closed")
		  ...
		  ?> <td class = "t1">
		  <form method="post" action="cardetail.php"  >
		  <select name="editstatus"><option value="closed" selected>closed</option><option value="cancel">cancel</option>
			  </select><input type="submit" value="update"></td> </tr></form>
...
<?php
		...
		if($select4Result)
		{
		   ...
		   $amid2 = $row['MUserID'];
			...
			$deptName3 = "";
			$amSec = "";
			$grt = "select DeptID from tblMUAccount where Name = '$amName'";
			...
			while($row = mysql_fetch_array($select6Result))
			{
				...
				*/
				...
				while($rca = mysql_fetch_array($gcar))
				{
					...
					$display2 .= "<form method=\"post\" action=\"cardetail.php\"><tr><td valign=\"top\">$ac</td><td valign=\"top\">$dbnm</td><td valign=\"top\">";
					
			       if($view != 1)
	               {
             
			        	$display2 .= "<select name=\"castatus\">";
			        	if($cas == "open")
			        	{
							$display2 .= "<option value=\"open\" selected>open</option><option value=\"closed\">close</option>";
			        	} 
			       		else if($cas == "closed")
			       		{
				   			$display2 .= "<option value=\"closed\" selected>close</option><option value=\"open\">open</option>";
			      		}
			        		$display2 .= "</select><input type=\"submit\" value=\"update\">";
			      }
			      else
			      {
			  			print $cas;
					...
					$display2 .= "</td><td valign=\"top\">";
					if($cas == "closed")
					{
					$display2 .= "$cafd";
					}
					$display2 .= "</td><input type =\"hidden\" name=\"what\" value=\"editstat\">";
					$display2 .= "<input type =\"hidden\" name=\"view\" value=\"view\">";
					$display2 .= "<input type =\"hidden\" name=\"caid\" value=\"$caid\">";
				...
				if($select1Result)
				{
					while($row = mysql_fetch_array($select1Result))
					{
						...
						$display2 .= "<td class =\"t1\"><a href=\"verification.php?flag=edit&inum=". $inum . "&sub=" . $row['sub'] .  "\">".$row['VERIFICATION']."</a></td>";
						$display2 .= "</tr>";
				...
				*/
				...
				if($status != "closed")
				{
				$display2 .= "<form method=\"post\" action=\"editcardtl.php\">";
				$display2 .= "<input type =\"hidden\" name=\"group\" value=\"$group\">";
				$display2 .= "<input type =\"hidden\" name=\"id\" value=\"$id11\">";
				$display2 .= "<input type =\"hidden\" name=\"inum\" value=\"$inum\">";
