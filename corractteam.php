...
include ("redirectlogin.php");
    ...
    $offsetOriginal = $offset; 
		...
		if(isset($_POST['what']))
		{
			...
			{
				$select .=" WHERE MUsername='$uname' AND Name LIKE '%$nm%' AND DeptID = $dpt AND GroupID = 'cat'";
			}
		
			//username is not empty
			if(($_POST['uname'] != "") && ($_POST['nm'] == "")&& ($_POST['sdept'] == ""))
			{
				$select .=" WHERE MUsername='$uname' AND GroupID = 'cat'";
			}
		
			//name is not empty
			if(($_POST['uname'] == "") && ($_POST['nm'] != "")&& ($_POST['sdept'] == ""))
			{
				$select .=" WHERE Name LIKE '%$nm%' AND GroupID = 'cat'";
			}
		
			//departemen is not empty
			if(($_POST['uname'] == "") && ($_POST['nm'] == "")&& ($_POST['sdept'] != ""))
			{
				$select .=" WHERE DeptID = $dpt AND GroupID = 'cat'";
			}
		
			//username and name are not empty
			if(($_POST['uname'] != "") && ($_POST['nm'] != "")&& ($_POST['sdept'] == ""))
			{
				$select .=" WHERE MUsername='$uname' AND Name LIKE '%$nm%' AND GroupID = 'cat'";
			}
			
			//username and departement are not empty
			if(($_POST['uname'] != "") && ($_POST['nm'] == "")&& ($_POST['sdept'] != ""))
			{
				$select .=" WHERE MUsername='$uname' AND DeptID = $dpt AND GroupID = 'cat'";
			}
		
			//name and departemen are not empty
			if(($_POST['uname'] == "") && ($_POST['nm'] != "")&& ($_POST['sdept'] != ""))
			{
				$select .=" WHERE Name LIKE '%$nm%' AND DeptID = $dpt AND GroupID = 'cat'";
			}
			
			if(($_POST['uname'] == "") && ($_POST['nm'] == "")&& ($_POST['sdept'] == ""))
			{
				$select .=" WHERE GroupID = 'cat'";
			}
    ...
    else
	{
		...
		$select = "SELECT * FROM tblMUAccount WHERE GroupID = 'cat' ORDER BY Name ASC LIMIT $offset,50";
	}
...
<p></p>
  ...
  <?php
  $form = "<center>";
	$form .= "<form method=\"POST\" action=\"corractteam.php\">";
	$form .= "<table class =\"style1\">";
	$form .= "<tr>";
	$form .= "<td>Username :</td><td><select name=\"uname\"><option value =\"\"></option>";
	$gu = "select MUsername from tblMUAccount where GroupID ='cat'";
	...
	$gn = "select Name from tblMUAccount where GroupID ='cat'";
    ...
    $getDept = "select DeptID from tblMUAccount where GroupID = 'cat'";
    $getDeptResult = mysql_query($getDept);
   ...
   <table width="78" border="0">
     <tr>
       <td><form name="form2" method="post" action="corractteam.php">
	   <input type="hidden" name="group" value="<?php echo $group; ?>">
...
{ 
     ...
     if ($offset < $staffCount) { 
          ...
          $displayNext .= "<td><form name=\"form1\" method=\"post\" action=\"corractteam.php\">";
...
{ 
     ...
     { 
          ...
          $displayPrevious .= "<td><form name=\"form1\" method=\"post\" action=\"corractteam.php\">";
...
{ 
    ...
    $countQuery = "SELECT count(MUserID) as staffCount FROM tblMUAccount WHERE GroupID = 'cat'";
