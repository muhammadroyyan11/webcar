...
if(isset($_POST['what']))
{
	if($_POST['what'] == "update")
	{
		...
		$update = "update tblCARDTL1 set CAMStat = '$action' where IssNumber='$inum' AND CAMID=$camid";
		$updateResult = mysql_query($update);
		if($updateResult)
		{
			print "<p class=\"style2\">Selected Corrective Action Manager status has been updated</p>";
		}
		else
		{
			print mysql_error();
...
<table border="0" class="style1">
	...
	<td><?php print $form3;?></td>
      <td><form name="form1" method="post" action="cam.php">
...
if($select5Result)
		{
			...
			while($row = mysql_fetch_array($select5Result))
			{
			    ...
			    //$display .= "<form method=\"post\" action=\"camcardetail.php\">";
				...
				if($id11 == $camid2)
				{
				$display .= "<td><select name=\"action\"><option value=\"approved\" selected>approve</option><option value=\"rejected\">reject</option></select></td>";
				
				$display .= "<input type=\"hidden\" name=\"id\" value=\"$id11\">";
				$display .= "<input type=\"hidden\" name=\"group\" value=\"$group\">";
		        $display .= "<input type=\"hidden\" name=\"inum\" value=\"$issNumber1\">";
				$display .= "<input type=\"hidden\" name=\"camid\" value=\"$camid2\">";
				$display .= "<input type=\"hidden\" name=\"what\" value=\"update\">";
		        $display .= "<td><input type =\"submit\" value=\"Submit\"></td>";
				}
				$display .= "</tr>";
				 $display .= "</form>";
				 */
			...
			*/
			while($row = mysql_fetch_array($select6Result))
			{
				...
				*/
				...
				$display2 .= "<form method=\"post\" action=\"cameditdtl.php\">";
				$display2 .= "<tr valign=\"top\" align = \"center\"><td colspan = \"2\" class = \"t2\">$inum</td>";
				$display2 .= "<td width=\"500\" class = \"t2\">Findings</td>";
				$display2 .= "<td width=\"500\" class = \"t2\">Analysis</td>";
				$display2 .= "<td width=\"500\" class = \"t2\">Corrective Action</td>";
				$display2 .= "<td class = \"t2\">Date Inspected</td><td class = \"t1\">$dateInspect</td></tr>";
				$display2 .= "<tr valign=\"top\" align = \"center\"><td class = \"t2\">Due Date</td><td class = \"t1\">$dueDate</td>";
				$display2 .= "<td rowspan=\"4\" width=\"500\" class = \"t1\">$findings</td>";
				$display2 .= "<td rowspan=\"4\" width=\"500\" class = \"t1\">$analysis</td>";
				$display2 .= "<td rowspan=\"4\" width=\"500\" class = \"t1\">$corrAct</td></td>";
				$display2 .= "<td class = \"t2\">Follow Up By Originator :</td><td class = \"t1\">$remark</td></tr>";
				$display2 .= "<tr valign=\"top\" align = \"center\"><td class = \"t2\">Status</td><td class = \"t1\">$carStatus</td>";
				$display2 .= "<td class = \"t2\">Closed Date :</td><td class = \"t1\">$closedDate</td></tr>";
				$display2 .= "<tr align = \"center\"><td class = \"t2\">ISO Ref</td><td class = \"t1\">$iso</td>";
				$display2 .= "<td class = \"t2\">Last Update By :</td>";
				$pf = "";
				if($pupdate != "")
				{
				...
				if($pf != "")
				{
					$display2 .= "<td class = \"t1\">$pf</td></tr>";
				}
				else
				{
					$display2 .= "<td class = \"t1\"></td></tr>";
				}
				$display2 .= "<tr valign=\"top\" align = \"center\"><td class = \"t2\">Category</td><td class = \"t1\">$category</td>";
				$display2 .= "<td class = \"t2\">Date :</td>";
				if(($dupdate != "")&&($mupdate != "")&&($yupdate != ""))
				{
					$ldate = $dupdate."/".$mupdate."/".$yupdate;
					$display2 .= "<td class = \"t1\">$ldate</td></tr>";
				}
				else
				{
					$display2 .= "<td class = \"t1\"></td></tr>";
				...
				*/
				...
				$display2 .= "<input type =\"hidden\" name=\"what\" value=\"view\">";
			    $display2 .= "<tr><td colspan=\"7\"><input type =\"submit\" value=\"Edit PIC\"></td></tr>";
				$display2 .= "</form>";
			}
			$display2 .="</table>";
