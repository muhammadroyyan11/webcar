...
include ("redirectlogin.php");
    ...
    $offsetOriginal = $offset; 
		...
		if(isset($_POST['what']))
		{
			...
			{
				$select .=" WHERE MUsername='$uname' AND Name LIKE '%$nm%' AND DeptID = $dpt AND GroupID = 'os'";
			}
		
			//username is not empty
			if(($_POST['uname'] != "") && ($_POST['nm'] == "")&& ($_POST['sdept'] == ""))
			{
				$select .=" WHERE MUsername='$uname' AND GroupID = 'os'";
			}
		
			//name is not empty
			if(($_POST['uname'] == "") && ($_POST['nm'] != "")&& ($_POST['sdept'] == ""))
			{
				$select .=" WHERE Name LIKE '%$nm%' AND GroupID = 'os'";
			}
		
			//departemen is not empty
			if(($_POST['uname'] == "") && ($_POST['nm'] == "")&& ($_POST['sdept'] != ""))
			{
				$select .=" WHERE DeptID = $dpt AND GroupID = 'os'";
			}
		
			//username and name are not empty
			if(($_POST['uname'] != "") && ($_POST['nm'] != "")&& ($_POST['sdept'] == ""))
			{
				$select .=" WHERE MUsername='$uname' AND Name LIKE '%$nm%' AND GroupID = 'os'";
			}
			
			//username and departement are not empty
			if(($_POST['uname'] != "") && ($_POST['nm'] == "")&& ($_POST['sdept'] != ""))
			{
				$select .=" WHERE MUsername='$uname' AND DeptID = $dpt AND GroupID = 'os'";
			}
		
			//name and departemen are not empty
			if(($_POST['uname'] == "") && ($_POST['nm'] != "")&& ($_POST['sdept'] != ""))
			{
				$select .=" WHERE Name LIKE '%$nm%' AND DeptID = $dpt AND GroupID = 'os'";
			}
			
			if(($_POST['uname'] == "") && ($_POST['nm'] == "")&& ($_POST['sdept'] == ""))
			{
				$select .=" WHERE GroupID = 'os'";
			}
    ...
    else
	{
		...
		$select = "SELECT * FROM tblMUAccount WHERE GroupID = 'os' ORDER BY Name ASC LIMIT $offset,50";
	}
...
<p></p>
  ...
  <?php
  $form = "<center>";
	$form .= "<form method=\"POST\" action=\"ostaff.php\">";
	$form .= "<table class =\"style1\">";
	$form .= "<tr>";
	$form .= "<td>Username :</td><td><select name=\"uname\"><option value =\"\"></option>";
	$gu = "select MUsername from tblMUAccount where GroupID ='os'";
	...
	$gn = "select Name from tblMUAccount where GroupID ='os'";
    ...
    $getDept = "select distinct DeptID from tblMUAccount where GroupID = 'os'";
    $getDeptResult = mysql_query($getDept);
   ...
   if($totalRows == 0)
		{
		print "<p class = \"style2\"><center>Originator has not been added yet</center></p>";
		} 
   ...
   <table width="78" border="0">
     <tr>
       <td><form name="form2" method="post" action="ostaff.php">
	   <input type="hidden" name="group" value="<?php echo $group; ?>">
...
{ 
     ...
     if ($offset < $staffCount) { 
          ...
          $displayNext .= "<td><form name=\"form1\" method=\"post\" action=\"ostaff.php\">";
...
{ 
     ...
     { 
          ...
          $displayPrevious .= "<td><form name=\"form1\" method=\"post\" action=\"ostaff.php\">";
...
{ 
    ...
    $countQuery = "SELECT count(MUserID) as staffCount FROM tblMUAccount WHERE GroupID = 'os'";
